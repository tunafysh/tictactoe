...
export default function Board({ tiles, onTileClick, playerTurn, strikeClass, gameState, isMobile }: {tiles?: string[], onTileClick: (i: number) => void, playerTurn: string, strikeClass: string, gameState: {}, isMobile: boolean}) {
    ...
    return (
        <div className={`relative cursor-pointer ${boardVisible ? 'grid' : 'hidden'} grid-rows-3 grid-cols-3`}>
            ...
            <Tile playerTurn={playerTurn} onClick={() => onTileClick(4)} value={tiles[4]} className="border-b-2 border-r-2 border-zinc-400" gamestate={gameState}/>
            <Tile playerTurn={playerTurn} onClick={() => onTileClick(5)} value={tiles[5]} className="border-b-2 border-zinc-400" gamestate={gameState}/>
            <Tile playerTurn={playerTurn} onClick={() => onTileClick(6)} value={tiles[6]} className="border-r-2 border-zinc-400" gamestate={gameState}/>
            <Tile playerTurn={playerTurn} onClick={() => onTileClick(7)} value={tiles[7]} className="border-r-2 border-zinc-400" gamestate={gameState}/>
            <Tile playerTurn={playerTurn} onClick={() => onTileClick(8)} value={tiles[8]} gamestate={gameState}/>
